...
const ShoppingCartProvider = ({ children }: any) => {
    ...
    const addProduct = (product: IProduct) => {
        ...
        if(isBrowser) {
            sessionStorage.seItem(SESSION_STORAGE, JSON.stringify(products))
        }
    }
    ...
    const deleteProduct = (id: string): void => {
        ...
        if(isBrowser) {
            sessionStorage.seItem(SESSION_STORAGE, JSON.stringify(newProducts))
        }
    }
    ...
    return (
        <ShoppingCartContext.Provider value={{ addProduct, getProducts, deleteProduct, getTotalValue, getTotalProducts, getShippingValue, clearAll }}>
            { children }
